...
public class UsuariosDao {
    ...
    // 
    public int Registrar(UsuariosVo v) throws SQLException {
        //consulta preparada
        sql="INSERT INTO usuarios (`nombreGenero`, `estadoGenero`) values (?,?)";
        try {
    ...
    public List<UsuariosVo>Listar() throws SQLException {
        ...
        List<UsuariosVo> ListaRoles= new ArrayList<>();
    ...
    public void Eliminar(int idRol) throws SQLException {
        sql="DELETE FROM usuarios WHERE idGenero="+idRol;
        try {
            con=Conexion.conectar();
            ps=con.prepareStatement(sql);
            ps.executeUpdate();
    ...
    public void activar(int id) throws SQLException {
        sql="UPDATE usuarios SET estadoGenero='Activo' WHERE idGenero="+id;
        try {
            con=Conexion.conectar();
            ps=con.prepareStatement(sql);
            ps.executeUpdate();
    ...
    public void inactivar(int id) throws SQLException {
        sql="UPDATE `usuarios` SET `estadoGenero`='Inactivo' WHERE idGenero ="+id;
        try {
            con=Conexion.conectar();
            ps=con.prepareStatement(sql);
            ps.executeUpdate();
    ...
    public void actualizar(int id, String genero, String estado) throws SQLException {
        sql="UPDATE `usuarios` SET `nombreGenero`='"+genero+"',`estadoGenero`='"+estado+"' WHERE idGenero="+id;
        try {
            con=Conexion.conectar();
            ps=con.prepareStatement(sql);
            ps.executeUpdate();
